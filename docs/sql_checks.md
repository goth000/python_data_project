...
Airflow запускает `load.py --mode incremental`. В одной транзакции скрипт
удаляет строки загружаемого периода и города, затем вставляет их снова.
Уникальный индекс `(date, city_id)` служит дополнительной защитой.

После повторного запуска одного периода:
...
Ожидаемый результат:
- 0 строк;
- количество строк загруженного периода остается стабильным.
