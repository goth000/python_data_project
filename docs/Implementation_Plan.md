...
По измерениям погоды часа `t` прогнозируется температура следующего часа
`t+1`. Такой прогноз может использоваться как простая аналитическая оценка
краткосрочного изменения температуры.

Признаки содержат только информацию, доступную в момент прогноза:
...
Target — температура следующего часа — формируется сдвигом и не входит в
признаки.

## Защита от Leakage

В ноутбуке показаны прямой leakage через target в признаках и ошибка обучения
preprocessing до split. Для проектной модели строки сортируются по времени,
первые 80% используются для train, последние 20% — для test. Перемешивание
временного ряда не выполняется.
...
* MAE — средняя абсолютная ошибка в градусах;
* RMSE — сильнее штрафует крупные ошибки;
* R² — показывает объяснённую моделью долю вариации относительно константного
  прогноза.

Linear Regression получила MAE около `0.73°C`, baseline — около `2.64°C`.
При этом R² модели около нуля, поэтому на семидневной истории модель нельзя
считать стабильной или готовой к практическому использованию.

## Артефакты

Скрипт `src/analytics/week13_ml.py` формирует:

```text
docs/ml/week13_summary.md
docs/ml/predictions_sample.csv
docs/ml/metrics.csv
docs/ml/predictions.png
docs/ml/metrics.png
```
...
LLM является отдельным report-шагом после вычисления данных. Он не участвует в
extract, transform, DQ или load и не считается источником чисел.

## Ограниченный контекст

`src/analytics/llm_summary.py` получает только MART, DQ summary и identity
варианта. Pandas рассчитывает период, количество строк, температурные метрики,
осадки, дождливые часы и максимальный ветер. Результат сохраняется в
`docs/llm/context.json`; raw-данные и приватная информация не передаются.

## Анти-галлюцинация

Сводка формируется в offline-режиме с явными ограничениями. После генерации
валидатор извлекает все числовые значения и проверяет, что они присутствуют в
рассчитанном контексте. Неожиданное число приводит к FAIL.
...
docs/llm/summary.md
docs/llm/validation.json
docs/LLM_Usage_Log.md
```

API-ключи не коммитятся. `.env` находится в `.gitignore`, а `.env.example`
содержит только пустые значения и подсказки.
